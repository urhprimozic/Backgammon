 ...
 * 
 * @Pair(v, p)
 * v - quality of the board from {@code [-1,1]},
...
public class Hevristic {
    ...
    public Pair<float[], Float> get(Board board, Pair<Integer, Integer> dice, int player){
        ...
        for (int t = 0; t< 24; t++){
            ...
            //check if a triangle has just one 
            if (board.board[t][0] == 1)v -= player * board.board[t][1] * badConstant;
            else if (board.board[t][0] == 0)continue;
            else{
