...
/**
 * A collection of static methods and variables, needed for the proper working of the game loop.
 * The needed game components are:
 * <ul>
 ...
 * </ul>
 * 
 * Alongside the game components, the following variables keep track of the game state:
 * <ul>
 * <li> <code>playerType</code> a map keeping track of computer and human players
 * <li> <code>player</code> the current player
 * <li> <code>movesMade</code> the amount of moves the current player has made this turn
 * <li> <code>humanRound</code> whether the current player is a human player
 * <li> <code>diceRolled</code> whether the dice have been rolled already for this turn
 * <li> <code>legalMoves</code> a list of all possible move orders the current player can make after the dice have been rolled
 * <li> <code>movesPlayed</code> the moves made this turn
 * <li> <code>maxMoves</code> the number of moves the player must make, given by the longest sequence of legal moves
 * <li> <code>comp1</code> and <code>comp2</code> the computer opponents, one for each player spot
 * </ul>
 */
public class Leader {
    ...
    /**
     * Resets the state to the starting position and starts the game loop.
     * This includes:
     * <ul>
     ...
     * <li> setting the {@link #AI AI} opponents with the time limit that the user has selected from {@link #MainFrame MainFrame}
     ...
     * 
     * This is done by refreshing the {@link MainFrame}, checking whether the game is over and setting <code>humanRound</code>
     * based on if a human or computer is playing this turn. If it is a human player's turn, the user interface calls both
     * <code>rollDice</code> and <code>playHumanMove</code> methods, as their calls are tied to human interactions with the interface.
     * If it is the computer's move, the <code>computerTurn</code> method is called.
     */
    ...
    /**
     * <p>Recieves the move to be played, executes it and updates the current state. Upon updating the state it calls
     * the <code>play</code> method to return to the game loop.</p>
     * 
     * Should the move be <code>null</code>, this is taken as there being no legal moves and the player is passing the turn.
     * 
     * @param move a <code>Pair</code> consisting of the start and end position of the chip
     */
    ...
    /**
     * <p>Handles the entire computer player's turn.</p>
     * 
     * This is done by constructing a <code>SwingWorker</code> where the
     * <code>doInBackground</code> and <code>done</code> methods are overwritten. The new <code>doInBackground</code> method
     * rolls the dice, refreshes the user interface and requests the move order from the <code>AI</code>. The new <code>done</code>
     * method receives the computed move order and executes each move in order. At the end it updates state variables. 
     */
